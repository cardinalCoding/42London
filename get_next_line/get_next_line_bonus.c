...
/*   Updated: 2023/12/13 05:20:03 by bcorte-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_move(char *start)
{
	...
	i += (start[i] == '\n');
	buff = malloc(1 + ft_strlen(start) - i);
	if (!buff)
		return (NULL);
	j = 0;
	while (start[i + j])
...
char	*get_next_line(int fd)
{
	char		*tmp;
	int			fd_read;
	static char	*start_str[1000] = {NULL};
	...
	free(tmp);
	tmp = ft_read_line(start_str[fd]);
	start_str[fd] = ft_move(start_str[fd]);
	return (tmp);
}
